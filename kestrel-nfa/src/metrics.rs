...
use ahash::AHashMap;
use kestrel_core::metrics_reporter::{MetricDescriptor, MetricId, MetricKind, MetricsReporter};
...
pub struct NfaMetrics {
    ...
    /// Optional metrics reporter for external export
    reporter: Option<Arc<dyn MetricsReporter>>,

    /// Metric ID for total events processed
    m_events_processed: MetricId,

    /// Metric ID for total alerts
    m_alerts: MetricId,
...
impl NfaMetrics {
    ...
    pub fn with_reporter(reporter: Option<Arc<dyn MetricsReporter>>) -> Self {
        let m_events_processed = reporter.as_ref().map_or(MetricId::NULL, |r| {
            r.register(MetricDescriptor {
        ...
        let m_alerts = reporter.as_ref().map_or(MetricId::NULL, |r| {
            r.register(MetricDescriptor {
        ...
        let m_sequences_active = reporter.as_ref().map_or(MetricId::NULL, |r| {
            r.register(MetricDescriptor {
    ...
    pub fn record_event(&self) {
        ...
        if let Some(ref reporter) = self.reporter {
            reporter.counter_inc(self.m_events_processed, 1);
        }
    }
    ...
    pub fn record_alert(&self) {
        ...
        if let Some(ref reporter) = self.reporter {
            reporter.counter_inc(self.m_alerts, 1);
        }
    }
