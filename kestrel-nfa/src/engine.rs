...
pub struct NfaEngine {
    ...
    m_alerts_generated: MetricId,
    m_partial_matches: MetricId,
    m_evictions: MetricId,
    m_budget_exceeded: MetricId,
}

impl NfaEngine {
    /// Create a new NFA engine with a metrics reporter.
    pub fn with_metrics_reporter(
        config: NfaEngineConfig,
        predicate_evaluator: Arc<dyn PredicateEvaluator>,
        metrics_reporter: Arc<dyn MetricsReporter>,
    ) -> Self {
        let nfa_metrics = NfaMetrics::with_reporter(Some(metrics_reporter.clone()));
        let metrics = Arc::new(RwLock::new(nfa_metrics));
        ...
        let m_events_processed = metrics_reporter.register(MetricDescriptor {
            name: "kestrel_nfa_events_processed_total",
            help: "Total events processed by the NFA engine",
            kind: MetricKind::Counter,
            labels: vec![],
        });
        let m_sequences_active = metrics_reporter.register(MetricDescriptor {
            name: "kestrel_nfa_sequences_active",
            help: "Current number of active sequences in the NFA engine",
            kind: MetricKind::Gauge,
            labels: vec![],
        });
        let m_alerts_generated = metrics_reporter.register(MetricDescriptor {
            name: "kestrel_nfa_alerts_generated_total",
            help: "Total alerts generated by the NFA engine",
            kind: MetricKind::Counter,
            labels: vec![],
        });
        let m_partial_matches = metrics_reporter.register(MetricDescriptor {
            name: "kestrel_nfa_partial_matches_total",
            help: "Total partial matches in the NFA engine",
            kind: MetricKind::Counter,
            labels: vec![],
        });
        let m_evictions = metrics_reporter.register(MetricDescriptor {
            name: "kestrel_nfa_evictions_total",
            help: "Total evictions in the NFA engine",
            kind: MetricKind::Counter,
            labels: vec![],
        });
        let m_budget_exceeded = metrics_reporter.register(MetricDescriptor {
            name: "kestrel_nfa_budget_exceeded_total",
            help: "Total budget exceeded events in the NFA engine",
            kind: MetricKind::Counter,
            labels: vec![],
        });
