...
1. **EQL 编译器测试失败（5个）**
   - `test_arithmetic_operators`: 算术运算符未实现
   - `test_maxspan_durations`: Duration 解析不支持 "10s" 格式
   - `test_parse_sequence_with_maxspan`: maxspan 语法问题
   - `test_parse_with_complex_logic`: 复杂逻辑表达式解析
   - `test_parse_with_in_expression`: In 表达式解析

2. **eBPF 采集未闭环**
   - Ring buffer polling 仅框架代码，实际读取未实现
   - `start_ringbuf_polling()` 函数体为空（仅日志输出）
   - 无法从内核获取真实事件进行端到端测试

3. **规则执行链路缺口**
   - `DetectionEngine::eval_event()` 只处理 NFA 序列规则
   - 单事件规则（`event where ...`）未实现评估路径
   - EQL 规则未被编译并注入运行时

#### 🟡 P1 - 架构一致性问题

1. **Wasm Runtime `event_get_bool` 问题**
   - suggest.md 曾指出缺少此函数
   - 已实现但未充分测试

2. **Wasm Runtime 每次评估都实例化**
   - 当前每次 `pred_eval` 都创建新 Store/Instance
   - 高频调用下有显著性能开销

3. **Predicate ID 格式约定不一致**
   - `WasmEngine::evaluate` 期望 `"rule_id:predicate_index"`
   - 但 NFA 引擎可能使用其他格式

#### 🟢 P2 - 优化与完善

1. **EventBus 分区未实装**
   - 代码中有 `partitions` 配置，但单 worker 运行
   - 无法支撑 NFA 并行与锁竞争控制

2. **Event::get_field O(n) 扫描**
   - 字段查找性能可优化（用 HashMap 或排序+二分）

3. **离线可复现未完全验证**
   - Mock Time API 就绪，但缺少与 NFA 的集成测试

### 1.3 项目价值评估

| 目标 | 达成度 | 风险等级 | 说明 |
|------|--------|----------|------|
| **端侧高性能** | 70% | 🟡 中 | 基础框架完成，性能优化未验证 |
| **EQL 兼容子集** | 60% | 🔴 高 | 编译器测试失败影响核心功能 |
| **双运行时** | 90% | 🟢 低 | Wasm/Lua 架构完整，ABI 一致 |
| **实时阻断** | 20% | 🔴 高 | 尚未开始 (Phase 6) |
| **离线可复现** | 75% | 🟡 中 | 基础就绪，验证不充分 |
...
```
kestrel-schema:  ✅ 所有测试通过
kestrel-event:   ✅ 所有测试通过
kestrel-core:    ✅ 所有测试通过 (time: 8, replay: 4, eventbus: ?, alert: ?)
kestrel-rules:   ✅ 测试通过
kestrel-engine:  ✅ 1/1 测试通过
kestrel-runtime-wasm: ✅ 3/3 测试通过
kestrel-runtime-lua:  ✅ 测试通过
kestrel-eql:     ⚠️  13/18 测试通过 (5 失败)
kestrel-nfa:     ✅ 21/21 测试通过
